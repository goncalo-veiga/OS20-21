...
int dir_move_entry(int inumber,int inumber_move, char* dirname, char* movename){
    ...
    for (int i = 0; i < MAX_DIR_ENTRIES; i++) {
        ...
        if (strcmp(inode_table[inumber].data.dirEntries[i].name,dirname) == 0) {

            for(int j = 0; j < MAX_DIR_ENTRIES; j++){
                //procura espaço disponivel para o novo node
                if(inode_table[inumber_move].data.dirEntries[j].inumber == FREE_INODE){

                    //passa node antigo para o novo path
                    inode_table[inumber_move].data.dirEntries[j] = inode_table[inumber].data.dirEntries[i];
                    strcpy(inode_table[inumber_move].data.dirEntries[j].name, movename);

                    //limpa node antigo
                    strcpy(inode_table[inumber].data.dirEntries[i].name, "");
                    inode_table[inumber].data.dirEntries[i].inumber = FREE_INODE;
 ...
 */
void inode_print_tree(FILE *fout, int inumber, char *name) {
    ...
    if (inode_table[inumber].nodeType == T_DIRECTORY) {
        ...
        for (int i = 0; i < MAX_DIR_ENTRIES; i++) {
            if (inode_table[inumber].data.dirEntries[i].inumber != FREE_INODE) {
                ...
                inode_print_tree(fout, inode_table[inumber].data.dirEntries[i].inumber, path);
            }
        }
    }
}
